...
open class KYScanView: UIView {
    //扫码区域各种参数
    var viewStyle: KYScanViewStyle = KYScanViewStyle()

    //扫码区域
    var scanRetangleRect: CGRect = CGRect.zero

    //线条扫码动画封装
    var scanLineAnimation: KYScanLineAnimation?

    //网格扫码动画封装
    var scanNetAnimation: KYScanNetAnimation?
     ...
     */
    public init(frame: CGRect, vstyle: KYScanViewStyle ) {
        viewStyle = vstyle

        switch (viewStyle.anmiationStyle) {
        case KYScanViewAnimationStyle.LineMove:
            scanLineAnimation = KYScanLineAnimation.instance()
            break
        case KYScanViewAnimationStyle.NetGrid:
            scanNetAnimation = KYScanNetAnimation.instance()
            break
        case KYScanViewAnimationStyle.LineStill:
    ...
    func drawScanRect() {
        ...
        case KYScanViewPhotoframeAngleStyle.Outer:
            diffAngle = linewidthAngle/3//框外面4个角，与框紧密联系在一起

        case KYScanViewPhotoframeAngleStyle.On:
            diffAngle = 0

        case KYScanViewPhotoframeAngleStyle.Inner:
    ...
    //根据矩形区域，获取识别区域
    static func getScanRectWithPreView(preView: UIView, style: KYScanViewStyle) -> CGRect {
